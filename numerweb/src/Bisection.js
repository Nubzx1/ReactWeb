...
function Bisection() {
  ...
  let [XR, setXR] = useState(1.5)
  let [XL, setXL] = useState(2)
  let [XM, XMO] = useState(0)
  ...
  function BisectionCode() {
    ...
    while (iteration <= 1 || e(XM, XMO) > 0.000001) {
      ...
      tempChart.push({ x: XM.toFixed(6), y: answer(fc, XM).toFixed(6) })
      iteration++
    }
    Xs = setXs(XM)
    console.log('XM = ' + XM)
    ...
    <Layout>
      <Content style={{ margin: '0 16px' }}>
        ...
        <div style={{ padding: 24, background: '#fff', minHeight: 480 }}>
          ...
          <br />
          <Button type="primary" onClick={BisectionCode} >Submit</Button>
          <br />
          <br />
          X = {Xs.toFixed(6)}
          <br />
