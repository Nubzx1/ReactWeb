...
function CentralDif() {
    ...
    function CentralDifCode() {
        ...
        if (state === 1) {
            if (Diff === 1) {
                ANS = (answer(fc, X + H) - answer(fc, X-H)) / (H)
                E = Deri1fc(fc, X)
            }
            else if (Diff === 2) {
                ANS = ((answer(fc, X + (H))) - (2 * answer(fc, X)) + answer(fc, X-H)) / (H ** 2)
                E = Deri2fc(fc, X)
            }
            else if (Diff === 3) {
                ANS = ((answer(fc, X + (2 * H))) - (2 * answer(fc, X + (H))) + (2 * answer(fc, X - H)) - answer(fc, X-(2*H))) / (H ** 3)
                E = Deri3fc(fc, X)
            }
            else if (Diff === 4) {
                ANS = ((answer(fc, X + (2 * H))) - (4 * answer(fc, X + (H))) + (6 * answer(fc, X)) - (4 * answer(fc, X - H)) + answer(fc, X-(2*H))) / (H ** 4)
                E = Deri4fc(fc, X)
            }
            else {
                ANS = 0
            ...
            if (Diff === 1) {
                ANS = (-(answer(fc,X + (2 * H))) + 8 * answer(fc,X + H) - 8 * answer(fc,X - H) + answer(fc,X - (2 * H))) / (12 * H)
                E = Deri1fc(fc, X)
            }
            else if (Diff === 2) {
                ANS = (-(answer(fc,X + (2 * H))) + 16 * answer(fc,X + H) - 30 * answer(fc,X) + 16 * answer(fc,X - H) - answer(fc,X - (2 * H))) / (12 * (H**2))
                E = Deri2fc(fc, X)
            }
            else if (Diff === 3) {
                ANS = (-(answer(fc,X + (3 * H))) + 8 * answer(fc,X + (2 * H)) - 13 * answer(fc,X + H) + 13 * answer(fc,X - H) - 8 * answer(fc,X - (2 * H)) + answer(fc,X - (3 * H))) / (8 * (H**3))
                E = Deri3fc(fc, X)
            }
            else if (Diff === 4) {
                ANS = (-(answer(fc,X + (3 * H))) + 12 * answer(fc,X + (2 * H)) - 39 * answer(fc,X + H) + 56 * answer(fc,X) - 39 * answer(fc,X - H) + 12 * answer(fc,X - (2 * H)) - answer(fc,X - (3 * H))) / (6 * (H**4))
                E = Deri4fc(fc, X)
            }
            else {
                ANS = 0
        ...
        <Layout>
            <Content style={{ margin: '0 16px' }}>
                ...
                <div style={{ padding: 24, background: '#fff', minHeight: 480 }}>
                    ...
                    <br />
                    <br />
                    <Button type="primary" onClick={CentralDifCode} >Submit</Button>
                    <br />
                    <br />
                    ANS = {ANSs}
                    <br />
