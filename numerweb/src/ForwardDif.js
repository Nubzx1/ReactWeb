...
function ForwardDif() {
    ...
    function ForwardDifCode() {
        ...
        if (state === 1) {
            if (Diff  === 1) {
                ANS = (answer(fc , X  + H) - answer(fc, X)) / (H)
                E = Deri1fc(fc , X )
            }
            else if (Diff  === 2) {
                ANS = ((answer(fc , X  + (2 * H ))) - (2 * answer(fc , X  + H )) + answer(fc , X )) / (H  ** 2)
                E = Deri2fc(fc , X )
            }
            else if (Diff  === 3) {
                ANS = ((answer(fc , X  + (3 * H ))) - (3 * answer(fc , X  + (2 * H ))) + (3 * answer(fc , X  + H )) - answer(fc , X )) / (H  ** 3)
                E = Deri3fc(fc , X )
            }
            else if (Diff  === 4) {
                ANS = ((answer(fc , X  + (4 * H ))) - (4 * answer(fc , X  + (3 * H ))) + (6 * answer(fc , X  + (2 * H ))) - (4 * answer(fc , X  + H )) + answer(fc , X )) / (H  ** 4)
                E = Deri4fc(fc , X )
            }
            else {
                ANS = 0
            ...
            if (Diff === 1) {
                ANS = (- answer(fc, X + (2 * H)) + (4 * answer(fc, X + H)) - (3 * answer(fc, X))) / (2 * H)
                E = Deri1fc(fc, X)
            }
            else if (Diff === 2) {
                ANS = (- (answer(fc, X + (3 * H))) + (4 * answer(fc, X + (2 * H))) - (5 * answer(fc, X + H)) + (2 * (answer(fc, X)))) / (H ** 2)
                E = Deri2fc(fc, X)
            }
            else if (Diff === 3) {
                ANS = (- (3 * answer(fc, X + (4 * H))) + (14 * answer(fc, X + (3 * H))) - (24 * answer(fc, X + (2 * H))) + (18 * answer(fc, X + H)) - (5 * answer(fc, X))) / (2 * (H ** 3))
                E = Deri3fc(fc, X)
            }
            else if (Diff === 4) {
                ANS = (-(2 * answer(fc, X + (5 * H))) + (11 * answer(fc, X + (4 * H))) - (24 * answer(fc, X + (3 * H))) + (26 * answer(fc, X + (2 * H))) - (14 * answer(fc, X + H)) + (3 * answer(fc, X))) / (H ** 4)
                E = Deri4fc(fc, X)
            }
            else {
                ANS = 0
        ...
        <Layout>
            <Content style={{ margin: '0 16px' }}>
                ...
                <div style={{ padding: 24, background: '#fff', minHeight: 480 }}>
                    ...
                    <br />
                    <br />
                    <Button type="primary" onClick={ForwardDifCode} >Submit</Button>
                    <br />
                    <br />
                    ANS = {ANSs}
                    <br />
