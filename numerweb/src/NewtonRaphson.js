...
function Newton() {
  ...
  function NewtonCode() {
    ...
    const Derifc = (fc, X) => derivative(parse(fc),'x').evaluate({x:X})
    ...
    while (iteration <= 1 || e(X, XO) > 0.000001) {
      ...
      X = XO-(answer(fc,XO)/Derifc(fc,XO))
      tempData.push({
        iteration: iteration,
        x: X.toFixed(6),
        e: e(X, XO).toFixed(6)
    ...
    <Layout>
      <Content style={{ margin: '0 16px' }}>
        ...
        <Breadcrumb style={{ margin: '16px 0' }}>
          <Breadcrumb.Item>Newton-Raphson</Breadcrumb.Item>
        ...
        <div style={{ padding: 24, background: '#fff', minHeight: 480 }}>
          ...
          <br />
          <Button type="primary" onClick={NewtonCode} >Submit</Button>
          <br />
          <br />
          X = {Xs.toFixed(6)}
          <br />
